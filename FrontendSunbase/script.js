...
LoginForm.addEventListener("submit", function (e) {
  ...
  fetch(AUTH_API, {
    ...
    //       throw new Error(`HTTP error, Status : ${response.status}`);
    //     }
    //     return response.text(); // Use text() instead of json() to inspect the response
    //   })
    //   .then((text) => {
    //     console.log("Response Text:", text);
...
function sync() {
    console.log("sync fn called");
  let customersSync = [];
  const apiUrl = `http://localhost:8080/customer/sync`;

  const authToken = localStorage.getItem("jwtToken");
  console.log("this is authentication key "+authToken);
  fetch(apiUrl, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    ...
    .catch((error) => {
      // Handle errors during the fetch operation
      console.error("Error:Shubham", error);
    });
}
